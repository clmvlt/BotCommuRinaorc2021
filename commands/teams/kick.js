...
module.exports = async function (it) {
    ...
    var profil = profils.find(p=>p.id == user.id);
    ...
    leaveEmbed.setTitle("KICK");
    leaveEmbed.setDescription(messages.UserKickFromTeam.replace('$user', `<@${user.id}>`));
    ...
    it.reply({
        content: messages.UserKickFromTeam.replace('$user', `<@${user.id}>`),
        ephemeral: true
    })
